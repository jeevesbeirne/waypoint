...
export default function ReflectTab() {
  ...
  // Capture as action (uses shared AddToChecklistModal)
  const [showCaptureModal, setShowCaptureModal] = useState(false);
  const [captureTitle, setCaptureTitle] = useState('');

  const todayPrompt = dailyPrompts.find((p) => p.day === dayNumber);

  // Get this week's checklist items
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      } catch (_) { /* meetings table might not exist yet */ }
    } catch (e) {
  ...
  const handleCaptureAsAction = () => {
    // Pre-populate with first line of reflection if available
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Capture as checklist action — shared component */}
      <AddToChecklistModal
...
const styles = StyleSheet.create({
  ...
  captureModalContainer: { flex: 1, backgroundColor: colors.background },
  captureModalScroll: { padding: spacing.lg, paddingBottom: spacing['3xl'] },
  captureModalTitle: {
    fontSize: typography.sizes.xl,
    fontWeight: typography.weights.bold,
    color: colors.primary,
  ...
  captureChipRow: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm, marginBottom: spacing.sm },
  captureChip: {
    paddingHorizontal: spacing.sm,
    paddingVertical: 6,
    borderRadius: radii.full,
  ...
  captureChipSelected: { backgroundColor: colors.primary, borderColor: colors.primary },
  captureChipText: { fontSize: typography.sizes.xs, color: colors.text.secondary },
  captureChipTextSelected: { color: '#fff', fontWeight: '600' },
  captureNote: {
    fontSize: typography.sizes.xs,
    color: colors.text.muted,
    marginTop: spacing.md,
  ...
  captureBtns: { flexDirection: 'row', gap: spacing.md },
  captureCancelBtn: {
    flex: 1,
  ...
  captureCancelText: { color: colors.text.secondary, fontWeight: '600' },
  captureSaveBtn: {
    flex: 1,
    paddingVertical: spacing.md,
    borderRadius: radii.full,
  ...
  captureSaveText: { color: '#fff', fontWeight: typography.weights.bold },
});
